...
function colisaoComInimigo() {
    ...
    // Verificar colisão frontal (colisão lateral)
    if (
        ...
        // Verificar se a colisão é frontal (não está pulando sobre o inimigo)
        if (checarPersonagem.bottom <= checarInimigo.top + 10) {
            // Se o personagem estiver acima do inimigo (pulo), o inimigo é derrotado
            inimigo.style.display = "none";
            audioMoeda.play(); // Pode tocar um som de vitória, se desejar
        } else {
            // Caso contrário, colisão lateral - perde vida
...
function colisaoComBloco() {
    ...
    if (
        ...
        clearInterval(checarColisaoComBloco); // Parar de checar colisão temporariamente
        
        // Atualiza moedas e pontos
        moedasAtual++;
        moedas.textContent = moedasAtual;
        localStorage.setItem("moedasAtual", moedasAtual);
        pontosAtual += 10;
        ...
        // Movimentar o bloco para cima e depois voltar à posição original
        const posicaoInicial = 500; // Posição original do bloco
        bloco.style.top = (posicaoInicial - 20) + "px"; // Subir 20px
        
        setTimeout(() => {
            bloco.style.top = posicaoInicial + "px"; // Voltar para a posição original
        }, 100); // Volta após 100ms

        // Reativar checagem de colisão após um pequeno tempo
        setTimeout(() => {
            checarColisaoComBloco = setInterval(colisaoComBloco, 10);
        }, 500); // Espera 500ms antes de checar novamente
    }
}
